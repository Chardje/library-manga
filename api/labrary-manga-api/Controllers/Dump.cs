    ...
    public class DumpController : ControllerBase
    {
        ...
        public async Task<ActionResult<List<Manga>>> GetAllMangas()
        {
            var result = await _context
                .Mangas.Include(m => m.MangaAuthors)
        ...
        {
            var result = await _context.Authors.Include(a => a.MangaAuthors).ToListAsync();
            return Ok(result);
        }
        ...
        {
            var result = await _context.Chapters.Include(c => c.Manga).ToListAsync();
            return Ok(result);
        }
        ...
        {
            var result = await _context.Genres.Include(g => g.MangaGenres).ToListAsync();
            return Ok(result);
        }
