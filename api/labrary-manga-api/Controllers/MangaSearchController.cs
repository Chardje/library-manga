    ...
    public class MangaSearchController : ControllerBase
    {
        ...
        )
        {
            ...
            if (!string.IsNullOrWhiteSpace(title))
                query = query.Where(m =>
                    (!string.IsNullOrEmpty(m.Title) && EF.Functions.ILike(m.Title ?? string.Empty, $"%{title}%")) ||
                    (!string.IsNullOrEmpty(m.TitleUa) && EF.Functions.ILike(m.TitleUa ?? string.Empty, $"%{title}%"))
                );

            if (!string.IsNullOrWhiteSpace(author))
                query = query.Where(m => m.MangaAuthors.Any(ma => ma.Author != null && EF.Functions.ILike(ma.Author.Name ?? string.Empty, $"%{author}%")));
